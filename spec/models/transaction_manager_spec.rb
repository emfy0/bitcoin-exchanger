...
RSpec.describe TransactionManager, type: :model do
  ...
  context described_class::BlockstreamApi, :vcr do
    ...
    describe '#tx_by_id' do
      let(:tx) do
        described_class.tx_by_id('665db7e88690ad3e31f87c74b729b1d2569e1d93d24ab1c4537ffd8264b4bec2')
      end
  ...
  context '#build_transaction', :vcr do
    ...
    context 'should create a new transaction', :vcr do
      it 'should raise NotEnoughBalance if it is not enough balance' do
        invalid_tx = described_class.new(cur_code_send: 'UST',
                                         cur_code_get: 'BTC',
                                         to_send_value: '10',
      ...
      it 'should return instance of Bitcoin::Protocol::Tx' do
        expect(new_transaction.build_transaction).to be_a Bitcoin::Protocol::Tx
      end

      it 'should contain two outputs' do
        expect(new_transaction.build_transaction.out.count).to eq(2)
      end

      it 'should create new record in transactions db' do
        new_transaction.build_transaction
        expect(new_transaction.db_tx).to be_a Transaction
      end

      it 'should output contains correct sum for user' do
        expect(new_transaction.build_transaction.out[0].value).to eq(50_325)
      end

      it 'should output contains correct sum for market' do
        send_back_to_market =
          described_class::BlockstreamApi.addr_balace(new_transaction.addr) -
          (new_transaction.to_get_value.to_f * 100_000_000).to_i - described_class::MINER_FEE * 100_000_000

        expect(new_transaction.build_transaction.out[1].value).to eq(send_back_to_market)
      end
    end
  end
end
